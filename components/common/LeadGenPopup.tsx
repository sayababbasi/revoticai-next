...
// // //             onClick={(e) => e.stopPropagation()}
// // //           >
// // //             <button onClick={() => setIsOpen(false)} className="absolute top-4 right-4 text-gray-400 hover:text-white z-20" aria-label="Close modal">
// // //               <CloseIcon />
// // //             </button>
...
// // //                     if (res.ok) {
// // //                       alert("Discount Claimed! We'll contact you soon.");
// // //                       setIsOpen(false);
// // //                     } else {
...
// //     <AnimatePresence>
// //       {isOpen && (
// //         <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={() => setIsOpen(false)}>
// //           <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} transition={{ duration: 0.3, ease: "easeInOut" }} className="relative w-full max-w-3xl bg-neutral-900 border border-neutral-700 rounded-2xl shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
// //             <button onClick={() => setIsOpen(false)} className="absolute top-4 right-4 text-gray-400 hover:text-white z-20"><CloseIcon /></button>
...
const LeadGenPopup: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await emailjs.send(
        "service_toql1x3", // Service ID
        "template_jea4yog", // Template ID
        {
          name,
          email,
          phone,
          company: "N/A", // Not used here but keeps template compatible
          services: "Home Page Offer Lead",
          message: "User requested a 30% discount offer via popup.",
        },
        "IF4WbCZuXMDeKvn5O" // Public key
      );
